...
u_star<-function(N.,P.,params){
  ...
  u_star<-sqrt(m*P./(r*b*(1-N./K)))-(k+m*h*N.)/b
...
pdf("Figure 2.pdf",bg="white",width=10.5,height=10)
par(mar=c(5.1,4.1,2.1,1.1))
m<-matrix(c(1,1,2,2,5,
            1,1,2,2,5,
            3,3,4,4,5,
            3,3,4,4,5
),ncol=5,byrow=TRUE)
layout(mat=m, heights=c(0.225,0.225,0.225,0.225),widths=c(0.225,0.225,0.25,0.25,0.1))

parameters<-c(r=1,K=100,d=0.3,c=1,m=0.04,k=1,b=6,h=0,u=0)
Timesteps<-1e4
InitN<-1
InitP<-1
...
for(t in 1:Timesteps){
  N.dynam[t+1]<-N.dynam[t]*(1+G_N(parameters,u.dynam[t],N.dynam[t],P.dynam[t])*dt)
  P.dynam[t+1]<-P.dynam[t]*(1+G_P(parameters,u.dynam[t],N.dynam[t],P.dynam[t])*dt)
...
cont.N<-seq(0,parameters['K'],0.1)
cont.P<-seq(0,parameters['K'],0.1)
...
lines(N,prey.isocline.vec(parameters,N,"fixed"),col=rgb(0,0,1,0.5),lty=2,lwd=2.5)
segments(pred.isocline.vec(parameters,N[1],"fixed"),0,pred.isocline.vec(parameters,N[1],"fixed"),150,col=rgb(1,0,0,0.5),lty=2,lwd=2.5)
plot.IBP(parameters,N,"one","optimized",ylim=c(0,parameters['K']),lwd=2.5)
# lines(N.dynam,P.dynam,lwd=2)
x.incs<-y.incs<-11
...
for(i in seq_along(x.seq)){
	for(j in seq_along(y.seq)){
		...
		arrows(x,y,x+x.dir[i,j]*10,y+y.dir[i,j]*10,length = 0.025)
	}
}
mtext('A)',side=3, adj=0, cex=2, line=0.5)
...
mtext('B)',side=3, adj=0, cex=2, line=0.5)
mtext(expression('Optimal vigilance ('*italic(u)^'*'*')'),side=4,line=2.1,cex=1.25)
par(new=TRUE)
plot(P.dynam/N.dynam,type='l',axes=FALSE,lwd=2.5,xlab="",ylab="Predator-Prey Biomass Ratio",col="purple")
axis(2,pos=0)

par(mar=c(5.1,4.1,2.1,1.1))
parameters<-c(r=1,K=100,d=0.3,c=1,m=0.04,k=4,b=12,h=0,u=0)
Timesteps<-1e4
InitN<-1
InitP<-1
Initu<-u_star(InitN,InitP,parameters)
...
mtext('C)',side=3, adj=0, cex=2, line=0.5)
...
axis(1,pos=0, at=0:5*2e3,labels = c("0","2e3","4e3","6e3","8e3","1e4"))
axis(4,pos=Timesteps)
mtext('D)',side=3, adj=0, cex=2, line=0.5)
mtext(expression('Optimal vigilance ('*italic(u)^'*'*')'),side=4,line=2.1,cex=1.25)
par(new=TRUE)
plot(P.dynam/N.dynam,type='l',axes=FALSE,lwd=2.5,xlab="",ylab="Predator-Prey Biomass Ratio",col="purple")
axis(2,pos=0)
